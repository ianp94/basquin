...
func main() {
	...
	// Namespaced by design (docs/OPERATOR-DESIGN.md §6). The operator observes (P1) and later mutates
	// (P2) only its own namespace, so its cache/watches are scoped to WATCH_NAMESPACE — set by the
	// Deployment from the pod's namespace via the downward API. This matches a RoleBinding-scoped
	// ClusterRole: the standing privilege is one namespace, not the whole cluster. An empty value is
	// refused rather than silently defaulting to cluster-wide, so the trust boundary can't erode by a
	// missing env var.
	watchNamespace := os.Getenv("WATCH_NAMESPACE")
	if watchNamespace == "" {
		setupLog.Error(nil, "WATCH_NAMESPACE is not set; this operator is namespaced by design "+
			"(see docs/OPERATOR-DESIGN.md §6) and refuses to run cluster-wide")
		os.Exit(1)
	}
