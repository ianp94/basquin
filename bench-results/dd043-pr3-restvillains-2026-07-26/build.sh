#!/usr/bin/env bash
# The containerized Maven build entrypoint for DD-043 PR-3's rest-villains acceptance.
# Usage: build.sh <maven args...>      e.g. build.sh clean package -DskipTests
#
# Copied from bench-results/dd043-pr2-restvillains-2026-07-26/build.sh with ONE change: the
# ${EXTRA_DOCKER_ARGS:-} expansion before "$IMAGE" (the established variable — see
# bench-results/dd043-spikes-2026-07-24/env/build.sh:41). PR-3 needs it to mount the injector jar
# and to run with --network host so the container reaches the injected repository served on the
# host's 127.0.0.1 (S5's measured transport channel; see that spike's README, "Transport note").
#
