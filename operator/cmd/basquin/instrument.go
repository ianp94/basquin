...
// buildTarget constructs a BasquinTarget from the options. Pure (no cluster access) so it's
...
func buildTarget(o instrumentOpts) *basquinv1alpha1.BasquinTarget {
	spec := basquinv1alpha1.BasquinTargetSpec{
		DeploymentRef:   basquinv1alpha1.DeploymentReference{Name: o.deployment},
		Container:       o.container,
		JVMOptsVar:      o.jvmOptsVar, // "" => the CRD defaults it (JAVA_TOOL_OPTIONS)
		CoverageService: o.coverageService,
		Agents:          basquinv1alpha1.AgentsSpec{ThreadTracker: &o.threadTracker},
	}
	if o.coverageIncludes != "" {
		...
		spec.Agents.Coverage = basquinv1alpha1.CoverageSpec{Enabled: true, Port: port, Includes: o.coverageIncludes}
	}
	if o.invariantMode != "" || o.latencyMaxMs > 0 || o.heapDeltaMaxKb > 0 {
		spec.Invariants = basquinv1alpha1.InvariantsSpec{
...
func runInstrument(args []string) error {
	...
	fs.StringVar(&o.name, "name", "", "BasquinTarget name (default: the deployment name).")
	...
	fmt.Printf("Applied BasquinTarget %s/%s (deployment %q)\n", o.namespace, o.name, o.deployment)
...
func applyTarget(ctx context.Context, c client.Client, desired *basquinv1alpha1.BasquinTarget) error {
	var existing basquinv1alpha1.BasquinTarget
...
func waitForInjected(ctx context.Context, c client.Client, key types.NamespacedName, timeout time.Duration) error {
	...
	last := ""
	for {
		var t basquinv1alpha1.BasquinTarget
