 ...
 * Headers:
 * - X-Basquin-Invariant-Count: number of violations (if any)
 * - X-Basquin-Invariant-Detail: first violation detail (truncated)
 *
...
public class IterationFilter implements Filter {
    ...
    private static void writeInvariantHeaders(ServletResponse response) {
        try {
            BasquinMetrics.incRequests();
            if (response instanceof HttpServletResponse) {
                HttpServletResponse resp = (HttpServletResponse) response;
                List<String> v = Agent.getLastInvariantViolations();
                if (v != null && !v.isEmpty()) {
                    BasquinMetrics.addInvariantCount(v.size());
                    resp.setHeader("X-Basquin-Invariant-Count", String.valueOf(v.size()));
                    String first = v.get(0);
                    if (first != null && first.length() > 200) first = first.substring(0, 200);
                    resp.setHeader("X-Basquin-Invariant-Detail", first);
                }
            }
