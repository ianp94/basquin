...
// AgentsSpec selects which Basquin agents to inject. Each is independently toggleable.
...
// InvariantsSpec is passed through as -Dbasquin.invariant.* flags.
...
// BasquinTargetSpec defines the desired state of BasquinTarget.
type BasquinTargetSpec struct {
	...
	// Invariants configures the -Dbasquin.invariant.* flags.
	// +optional
...
// BasquinTargetStatus defines the observed state of BasquinTarget.
type BasquinTargetStatus struct {
...
// BasquinTarget is the Schema for the basquintargets API
type BasquinTarget struct {
	metav1.TypeMeta   `json:",inline"`
	metav1.ObjectMeta `json:"metadata,omitempty"`

	Spec   BasquinTargetSpec   `json:"spec,omitempty"`
	Status BasquinTargetStatus `json:"status,omitempty"`
}

//+kubebuilder:object:root=true

// BasquinTargetList contains a list of BasquinTarget
type BasquinTargetList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []BasquinTarget `json:"items"`
}

func init() {
	SchemeBuilder.Register(&BasquinTarget{}, &BasquinTargetList{})
}
