...
- [x] **Helm chart to deploy the operator** — `deploy/helm/closurejvm-operator/`: CRDs + namespaced RBAC (Roles, not ClusterRoles) + controller Deployment, with the three images wired from values onto the controller args (no manual patching). `INSTALL=helm deploy/e2e/e2e.sh` exercises it end to end (injection + campaign + dashboard, 0 RBAC forbidden). Follow-ups: **publish the chart so it installs without a clone** — preferred: a **GitHub Pages Helm repo** (chart-releaser packages the chart + `index.yaml` onto the existing Pages site, so `helm repo add closurejvm https://ianp94.github.io/closureJVM && helm install closurejvm/closurejvm-operator`); alternative/additional: **OCI push to `ghcr.io`** (`helm push ... oci://ghcr.io/ianp94/charts`, `helm install oci://...`). Also a Make target/CI check that the chart RBAC hasn't drifted from `make manifests`. *(publish-preference: user 2026-07-20)*
- [~] **CLI to launch tests** — a thin Go `closurejvm` CLI (reuses the operator's api/v1alpha1 types + controller-runtime client → applies real typed CRs). `make -C operator cli`.
  ...
  - [ ] `run` — create the grammar/corpus ConfigMaps from local files + apply a `ClosureJVMCampaign` (--target, --base-url, --iterations|--duration, --grammar, --corpus), tail status to completion.
