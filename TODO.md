...
- [x] **Helm chart to deploy the operator** — `deploy/helm/closurejvm-operator/`: CRDs + namespaced RBAC (Roles, not ClusterRoles) + controller Deployment, with the three images wired from values onto the controller args (no manual patching). `INSTALL=helm deploy/e2e/e2e.sh` exercises it end to end (injection + campaign + dashboard, 0 RBAC forbidden). **Publishing (done 2026-07-20):** chart published to a **GitHub Pages Helm repo** at `docs/charts/` (`deploy/helm/publish.sh` packages + indexes; `helm repo add closurejvm https://ianp94.github.io/closureJVM/charts`); the four **container images publish to ghcr.io** (`ghcr.io/ianp94/closurejvm-*`, chart defaults point there) via `.github/workflows/release.yml` on a `v*` tag, which also attaches the CLI binaries + packaged chart to the GitHub Release. Remaining follow-ups: cut the first real tag to verify the CI publish end-to-end; a Make target/CI check that the chart RBAC hasn't drifted from `make manifests`; multi-arch images (the agents `.so` is amd64-only); OCI-registry chart push as an alternative to the Pages repo.
- [~] **CLI to launch tests** — a thin Go `closurejvm` CLI (reuses the operator's api/v1alpha1 types + controller-runtime client → applies real typed CRs). `make -C operator cli`.
  ...
  - [x] Distribution: `.github/workflows/release.yml` cross-builds the CLI (linux/darwin, amd64/arm64) → GitHub Release assets on a version tag. Follow-up: a `kubectl closurejvm` plugin alias.
