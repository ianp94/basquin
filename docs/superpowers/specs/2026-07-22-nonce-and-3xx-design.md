...
- `fire` 3xx: `fireR(...)` returns `(302, "/Wiki.jsp?page=SessionExpired")` for a redirect handler and `(200, null)` for a plain handler; the `int` `fire(...)` delegates (same code). Driving a redirect through the worker increments `redirects` and the right `redirectTargets` key. **Plus a `302`-carrying-`Set-Cookie` test** asserting the jar still captures it (pins the DD-035 fix).
...
- `runner/coverage/LoadRun.java` — `substitute` `@nonce` branch + `RUN_SALT`/`NONCE`; `lintCorrelationOrdering` exempts `@nonce`; `fire` no-follow + a distinct `fireR(...)` returning `FireResult(code, location)` (the int `fire` delegates) + `redirects`/`redirectTargets` + static `normalizeLocation`; the worker loop substitutes path+body and records the redirect; `summaryJson` (hard-bounded top-N).
