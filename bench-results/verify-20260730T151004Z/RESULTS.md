# DD-043 PR-3 verification — 20260730T151004Z

Commit: `119d400` on `dd043-pr3-maven-injector` — tree clean at run start (`git-status.txt`)
...
|---|---|---|
| PASS | `unit` | 389 tests, 0 failures |
...
| PASS | `guards:restored` | source restored, module suite green (30 tests, 0 failures per guard-restore-junit/) |
...
| PASS | `jvm:boundary` | poll returned 1 formatted hop line(s): 783,3408,9\|0\|\| |
...
Poll returned `783,3408,9|0||` (`jvm-result-poll.txt`; per hop line:
latencyMs, heapDeltaKb, threadDelta \| invariant count \| detail \| leak).
Every heap delta in this poll is non-negative (sign derived from `jvm-result-poll.txt`, not hand-typed).
