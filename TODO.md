...
- [x] **Helm chart to deploy the operator** — `deploy/helm/closurejvm-operator/`: CRDs + namespaced RBAC (Roles, not ClusterRoles) + controller Deployment, with the three images wired from values onto the controller args (no manual patching). `INSTALL=helm deploy/e2e/e2e.sh` exercises it end to end (injection + campaign + dashboard, 0 RBAC forbidden). Follow-ups: publish to a Helm repo; a Make target/CI check that the chart RBAC hasn't drifted from `make manifests`.
