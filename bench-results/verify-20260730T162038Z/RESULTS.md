# DD-043 PR-3 verification — 20260730T162038Z

Commit: `04fc4a5` on `dd043-pr3-maven-injector` — tree clean at run start (`git-status.txt`)
...
| PASS | `jvm:boundary` | poll returned 1 formatted hop line(s): 765,-827,10\|0\|\| |
...
| PASS | `native:boundary` | poll returned 1 formatted hop line(s): 5,0,0\|0\|\| |
...
Poll returned `765,-827,10|0||` (`jvm-result-poll.txt`; per hop line:
latencyMs, heapDeltaKb, threadDelta \| invariant count \| detail \| leak).
**This poll carries negative heap delta(s) (`-827`) — a known gap, not a bug in this run:**
...
Poll returned `5,0,0|0||` (`native-result-poll.txt`; per hop line:
latencyMs, heapDeltaKb, threadDelta \| invariant count \| detail \| leak).
