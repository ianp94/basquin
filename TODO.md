...
- [ ] **`ClosureJVMCampaign` (test) CRD** — a second CRD that fires off a complete test run (DD-025, [`docs/CAMPAIGN-DESIGN.md`](CAMPAIGN-DESIGN.md)). Phased P5a–P5d.
  ...
  - [ ] **Driver coverage-classes: handle war-only target images** — the driver initContainer copies `WEB-INF/classes` **out of the target image** (§7b), but a war-packaged app ships `ROOT.war` with the exploded dir only existing at runtime, so the copy finds nothing and coverage is 0. The e2e works around this by exploding the war into `ROOT/` at image-build time. Operator options: extract classes from the war/lib jars, or copy from the *running* target pod instead of the image. *(surfaced by the campaign e2e)*
...
- [ ] **Deeper content-drift detection** — `injectionApplied` re-heals a *missing initContainer* but not a stripped/edited `jvmOptsVar` while the annotation + initContainer remain (surfaced by the e2e `kubectl apply`-over-injected case): the operator reads it as "still injected" and doesn't re-append the flags. Compare the actual injected env/volume against expected, not just the initContainer's presence. *(surfaced by the in-cluster e2e)*
